...
function Go()
	...
	-- look_around(typesStaff, clearTT, effect_range + 2)
...
function checkEnergy( e, distance )
	-- if distance > effect_range then
	-- 	e.Tooltip = nil
	-- 	return
	-- end
	-- e.Tooltip = {'staff_tt_energy', math.floor(e.Energy), 'N'}

	if e.drinking_id and e.drinking_id ~= this.Id.u and World.TimeIndex <= e.drinking_timeout then
			-- Leave e to that other drinkmachine
			return
	end

	if e.drinking_id ~= this.Id.u and storage_available <= 0 then return end
	if e.drinking_id == this.Id.u and entity_on_the_wayz[e] == nil then
		entity_on_the_wayz[e] = World.TimeIndex
		print('continue from loaded game')
	end
		-- Is or will be out of service
		-- Don't invite more unless they are already coming
	...
	-- Attract e to come here
	e.drinking_id = this.Id.u
	e.drinking_timeout = World.TimeIndex + 20

	storage_available = storage_available - 1
	-- storage_available = storage_available - (this.threshold / 10 - math.floor(e.Energy / 10))
	...
	if distance < 1.1 then
		...
		this.Sound("DrinkMachine", "Place") -- pong
		...
		entity_on_the_wayz[e] = nil
		-- Ready = World.TimeIndex + 0.6
		-- e.Tooltip = {'staff_tt_energy', math.floor(e.Energy), 'N', tostring(e.drinking_id), 'B'}
	else
		...
		-- local c = World.GetCell(math.floor(nav_x+0.5), math.floor(nav_y+0.5))
		...
		-- print(entity_on_the_wayz)
		-- if e.PlayerOrderPos then
		-- 	e.PlayerOrderPos.x, e.PlayerOrderPos.y = e.Dest.x, e.Dest.y
		-- end
	end
...
function nav(who, to, dx, dy)
	...
	-- who.PlayerOrderPos.x, who.PlayerOrderPos.y = (to.Pos.x + (dx or 0)), (to.Pos.y + (dy or 0))
end
...
function Init()
	...
	if World.StaffNeeds then
		...
		Interface.AddComponent(this, "dm__toggle_serves_g", 'Button', "dm__toggle_serves_g", tostring(this.serves_g), 'B')
	end
	...
	Interface.AddComponent(this, "dm__toggle_same_room", 'Button', "dm__toggle_same_room", tostring(this.needs_same_room), 'B')
end
